    ...
    public class AuthController : Controller
    {
        ...
        private async Task SignInUser(HttpContext context, User user)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(user.Email))
                    claims.Add(new Claim(ClaimTypes.Email, user.ID.ToString(), ClaimValueTypes.Email, GasAdminAuthenticationDefaults.ClaimsIssuer));

                //create principal for the current authentication scheme
                var userIdentity = new ClaimsIdentity(claims, GasAdminAuthenticationDefaults.AuthenticationScheme);
                ...
                // sign in
                await context.SignInAsync(GasAdminAuthenticationDefaults.AuthenticationScheme, userPrincipal, authenticationProperties);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Logout()
        {
            try
            {
                ...
                await Request.HttpContext.SignOutAsync(GasAdminAuthenticationDefaults.AuthenticationScheme);
